...
/**
 * 
 * @author Jonas J. Schreiber This code is used to listen on a socket (currently port 6881) and
 *         accept incoming peer connections. Some of this code is attributable to the author of
	...
	public void run() {
		...
		while (!manager.stopThreads) // need to break at some point, and close inputstream
		{
			try {
				...
				potentialPeer = new Peer(clientSocket.getPort(),
						clientSocket.getInetAddress().getHostAddress(), peerID);
				...
				if (!Tracker.containsPeer(manager.peers, potentialPeer)) {
					...
					Thread messageHandler = new Thread(
							new MessageHandler(manager, potentialPeer, torrentInfo));
