...
/**
 * Peer class contains all the states and methods associated with a "peer" as defined in the
 * BitTorrent protocol.
 * 
 * @author Dylan Murray
...
public class Peer {
	 ...
	 */
	public final static ByteBuffer KEY_PEERID = ByteBuffer
			.wrap(new byte[]{'p', 'e', 'e', 'r', ' ', 'i', 'd'});
	...
	/**
	 * The socket the client communicates with this peer through. Isn't initialized until connect()
	 * is called.
	 */
	private Socket socket;

	/**
	 * An array list of ints which correspond to the pieces available to us in the bitfield, for
	...
	private Long end;
	/**
	 * Integer value used to describe the peer's download speed. This metric is used in
	 * PerformanceAnalyzer.
	 */
	...
	/**
	 * Integer value used to describe the peer's upload speed. This metric is used in
	 * PerformanceAnalyzer.
	 */
	 ...
	 *            A byte array representing all the pieces we've got so far.
	 * @return
	 */
	public int getNeededPieceIndex(boolean[] completedPieces, boolean[] piecesInProg) {
		for (int i = 0; i < availablePieces.size(); i++) {
			if (completedPieces[availablePieces.get(i)] == false
					&& piecesInProg[availablePieces.get(i)] == false) {
	...
	/**
	 * Adds a single index to this peer's available piece list; typically will be called in response
	 * to a Have message.
	 */
	...
	/**
	 * An ArrayList of Messages that represents of a queue of messages that need to be sent out to
	 * this peer.
	 */
