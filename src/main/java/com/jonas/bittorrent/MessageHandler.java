...
/**
 * This class is used to facilitate communication between a "Manager" and each one of its' peers;
 * that is, for every Peer that Manager wants to connect to, a new MessageHandler is created.
 * MessageHandler is responsible for requesting and sending all the various methods and pieces of
 * the file.
 * 
 * @author Dylan Murray
...
public class MessageHandler implements Runnable {
	...
	/**
	 * Current Piece (so that the decode on the last piece doesn't think it is still standard
	 * pieceLength), is set to -1 when we are not currently in the middle of a piece reception.
	 */
	...
	/**
	 * Message sender responsible for sending messages to peer. Gets instantiated after handshaking
	 * with peer (after we're connected).
	 */
	...
	private void managePeerConnection() throws Exception {
		...
		// Keep connection with peer up sending/receiving messages until the user initiates the
		// shutdown sequence
		while (!manager.stopThreads) {
			try {
				...
				System.err.println(
						"EOFException from peer " + this.peer + "... disconnecting from peer.");
				disposeCurrPiece();
				closeConnections();
				return;
			}
	...
	private boolean manageHandshake() throws IOException {
		try {
			...
			// if(!Message.doHandshake(in, out, generateHandshake(),
			// this.torrentInfo.info_hash.array())){
			// //TODO: Handle a bad handshake/ unresponsive peer. Currently just quits program.
			// System.err.println("Handshake to peer: " + peer.toString() + " unsuccessful. Info
	 ...
	 * protocol'. After the fixed headers are 8 reserved bytes which are set to 0. Next is the
	 * 20-byte SHA-1 hash of the bencoded form of the info value from the metainfo (.torrent) file.
	 * The next 20-bytes are the peer id generated by the client. The info_hash should be the same
	 * as sent to the tracker, and the peer_id is the same as sent to the tracker. If the info_hash
	 * is different between two peers, then the connection is dropped.
	 * 
	 * All integers are encoded as 4-bytes big-endian (e.g. 1,234 should be encoded as (hex) 00 00
	 * 04 d2). The peer_id should be randomly-generated for each process your program generates.
	 * (Meaning that it should probably change in some way each time the client is run.) Stick to
	 * alphanumerics for easier debugging.
	 * 
	...
	/**
	 * Calculates the length of a piece to request (it'll typically just be the regular piece
	 * length, but if it's the last piece it's going to be a bit smaller.
	 * 
	 * @param pieceIndex
	 * @return
	 */
	...
	private void processMessage(Message message, Peer peer) throws Exception {
		...
		if (message.toString() != null)
			...
			if (manager.stopThreads)
				return;
		switch (message.id) {
			case Message.CHOKE : // Choke Message, let our peer object know that it is now choking
									// us.
				peer.setPeerChoking(true);
				peer.setInterested(false);
				System.out.println(peer + " is now choking us.");
				// We've been disconnected before we can get the full piece; throw out the partial
				// pieces we got.
				disposeCurrPiece();
				break;
			case Message.UNCHOKE : // Unchoke Message, let our peer object know that it is not
									// choking us.
				...
				break;
			case Message.INTERESTED : // Interested Message, let our peer object know that it is
										// interested in us.
				...
				break;
			case Message.UNINTERESTED : // Uninterested Message, let our peer object know that it is
										// uninterested in us.
			...
			case Message.HAVE :
				...
				System.out.println(peer + " has piece " + ((HaveMessage) message).pieceIndex
						+ ", adding to available pieces");
				if (currentPieceIndex == -1) {
					checkPeersPieces();
				}
				break;
			case Message.BITFIELD :
				...
				peer.setBitfield(bitfieldMessage.bitfield); // Sets the peer's bitfield as shown in
															// message.
				// If we aren't already in the middle of a piece, this should never happen since
				// bitfield is only sent after handshake, but just to be safe
				if (currentPieceIndex == -1) {
					checkPeersPieces();
				}
				break;
			case Message.REQUEST :
				if (peer.amChoking()) {
					System.out.println(
							peer + " is currently being choked. Ignoring Request message.");
				} else {
			...
			case Message.PIECE :
				...
				if (partialPieces.size() * subdivisionSize >= calcPieceLength(currentPieceIndex)) { // ensures
																									// concatenation
																									// of
																									// last
																									// piece
					...
					checkPeersPieces();
				} else { // We're still in the middle of getting all the parts of a piece; request
							// the next block
					sendNextRequestMessage(((PieceMessage) message).index,
							((PieceMessage) message).begin);
				}
				// System.out.println("Peer: " + this.peer.toString() + " is currently uploading to
				// us at: " + this.peer.getUploadSpeed() + "kB/s");
				break;
			case Message.CANCEL :
			...
			default :
				System.err.println(
						"Unknown message tried to be processed from peer " + this.peer + ".");
		}
	}
	...
	private void processRequestMessage(RequestMessage message) throws IOException {
		...
		if (message.length > 32768) {
			System.err
					.println(peer + " request a piece larger than 32KB. Disconnecting from peer.");
			disposeCurrPiece();
			closeConnections();
			return;
		}
		// we don't have the piece.
			...
			System.err.println(peer + " has requested a piece(" + message.index
					+ ") that we don't have! Disconnecting from peer.");
			disposeCurrPiece();
			closeConnections();
			return;
		}
		...
		// we're good to go
			...
			// System.out.println("Sending piece " + message.index + " at offset " + message.begin +
			// " to peer " + peer);
			...
			// System.out.println("Peer: " + this.peer.toString() + " is currently downloading from
			// us at: " + this.peer.getDownloadSpeed() + "kB/s");
		}
	}

	/**
	 * Disposes of any partially-downloaded pieces, and sets the inProg status of this index to
	 * false.
	 */
	...
	private synchronized void checkPeersPieces() throws IOException {
		...
		for (int i : manager.piecesByRarity) {
			...
			if (this.peer.availablePieces.contains(i)) {
				System.out
						.println("Peer " + this.peer + " has a desired piece at index " + i + "!");
				// if the peer is choking us and we haven't sent an interested message yet
				if (peer.isChoking() && !peer.amInterested()) {
					System.out.println("We're currently choked - sent interested message to peer "
							+ this.peer);
					addToQueue(new Message(Message.INTERESTED));
					peer.setInterested(true);
					sentMessage = true;
				} else if (!peer.isChoking()) {
					System.out.println("Sent RequestMessage for first block of piece index: " + i
							+ " to peer " + this.peer);
					...
					// The peer is choking us and we've already sent an interested message. Nothing
					// we can do.
					System.out.println(this.peer
							+ " is choking us and we've already sent an interested message.");
					sentMessage = true;
				}
			}
		}
	...
	/**
	 * Takes the index and begin of PREVIOUS request message and USES it to calculate the NEXT
	 * request message. The begin passed to this method is NOT the begin that will be requested.
	 * 
	 * @param index
	 * @param begin
	 * @throws IOException
	...
	private void sendNextRequestMessage(int index, int begin) throws IOException {
		...
		if (!(newOffset + subdivisionSize > calcPieceLength(currentPieceIndex))) {
			// System.out.println("Sending Request Message to " + this.peer + " for piece " + index
			// + " at offset " + newOffset);
			addToQueue(new RequestMessage(index, newOffset, subdivisionSize));
		} else {
			addToQueue(new RequestMessage(index, newOffset,
					calcPieceLength(currentPieceIndex) - newOffset));
		}
	}
	...
	private void concatenate() throws Exception {
		...
		if (manager.verifyPiece(temp)) // Verify the piece we received to be valid, and save it if
										// it is.
	...
	/**
	 * This is necessary because PerformanceAnalyzer and Peer don't have a handle on MessageSender
	 * but they do have a handle on MessageHandler, which has a handle on MessageSender. Therefore
	 * the call must go through the messageSender.
	 */
	...
	public void run() {
		try {
			...
			System.err.println(
					"Socket for peer: " + this.peer + " timed out. Disconnecting from peer.");
			try {
				closeConnections();
			} catch (IOException e) {
				System.err.println(
						"meta-exception, couldnt close connections after exception was already thrown.");
				e.printStackTrace();
			}
		} catch (IOException e) {
			e.printStackTrace();
