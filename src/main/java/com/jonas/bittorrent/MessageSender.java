...
/**
 * MessageSender is a thread instantiated in every MessageHandler who's duty is to send out all the
 * messages in a given peer's queue. This allows for true simultaneous sending and receiving.
 * 
 * @author Dylan Murray
 */
public class MessageSender implements Runnable {
	...
	public void run() {
		...
		while (active) {
			...
			// if there are messages to be sent, send the first message that was added to the queue
			// (FIFO)
			else {
				try {
					if (this.peer.messageQueue.get(0).id != (Message.PIECE)
							&& this.peer.messageQueue.get(0).id != (Message.REQUEST)) {
						System.out.println(
								"Sending " + this.peer.messageQueue.get(0) + " to " + this.peer);
					}
