...
/**
 * This class is used to interact with the file we're downloading to/ uploading from - it contains
 * methods to read and write specific pieces from the file.
 * 
...
public class FileAccess {
	...
	public FileAccess(Manager manager, TorrentInfo torrentInfo) throws Exception {
		...
		// Very important to see whether file already exists or is being created anew, as different
		// steps will be taken in each case.
		if (!file.exists()) {
	...
	/**
	 * Called upoon creation of a new torrent file. Ensures that RandomAccessFile will not try to
	 * access parts of the file that haven't been written. Also utilized in the TallyPieces()
	 * method.
	 * 
	 * @throws IOException
	 */
	...
	public boolean[] tallyPieces() throws IOException {
		...
		for (i = 0; i < torrentInfo.piece_hashes.length; i += 1) {
			...
			if (Arrays.equals(comparator, temp)) {
				myPieces[i] = false;
				// System.out.println("tallyPieces has found that piece: " + i + " is " +
				// myPieces[i]);
			} else {
				byte[] piece;
				if (i == torrentInfo.piece_hashes.length - 1) {
					piece = new byte[torrentInfo.file_length % torrentInfo.piece_length];
					// System.out.println("Last Piece, piece index: " + i + " is " + piece.length +
					// " bytes long.");
				} else
