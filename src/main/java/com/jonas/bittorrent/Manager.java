...
import com.jonas.bittorrent.gui.PeerDisplay;

/**
 * This class handles the overall flow of the application. When the tracker returns peers, it
 * checks, configures, and starts them. When pieces are completed by a peer it validates and saves.
 * It keeps track of which pieces we have and thus facilitates which pieces should be requested, and
 * when to choke or unchoke peers. Also has a Tracker with which it interacts.
 * 
 * @author Dylan Murray
...
public class Manager {
	...
	/**
	 * The list of valid peers (currently RUBT11) that have a piece of the file we're trying to
	 * download.
	 */
	public ArrayList<Peer> peers = new ArrayList<Peer>();

	/**
	 * A boolean array used to store all the pieces we download. This ensures that even when we
	 * download pieces out of order we have a way of organizing them.
	 */
	public boolean[] myPieces;

	/**
	 * Pieces in progress - any pieces that are currently being downloaded from a peer will be kept
	 * track of here.
	 */
	...
	/**
	 * A Thread object corresponding to the Tracker class This must be on its own separate Thread,
	 * as it sleeps for the min interval specified by the tracker. Therefore, to make the Tracker
	 * shut down in a timely fashion, Manager must have a handle on it, to call in the exit()
	 * method.
	 */
	...
	/**
	 * A TrackerStats object, which contains the statistics relevant to the tracker for use between
	 * sessions
	 */
	...
	/**
	 * Constructor for Manager class. Initializes instance variables, sets up Threads facilitates
	 * the connection to tracker and peers, and downloads the file.
	 * 
	 * @param dlPath
	 * @param torrentInfo
	 * @param peerID
	...
	public Manager(String dlPath, TorrentInfo torrentInfo, byte[] peerID) throws Exception {
		...
		this.socketListener = new SocketListener(this, this.torrentInfo); // SocketListener Thread
																			// Setup
		...
		this.pa = new PerformanceAnalyzer(this, torrentInfo.piece_hashes.length); // Performance
																					// Analyzer
																					// Thread Setup
		...
		Thread runtimeCommands = new Thread(new RuntimeCommands(this)); // RuntimeCommands Thread
																		// Setup
		runtimeCommands.setName("Runtime Commands");
		Runtime.getRuntime().addShutdownHook(runtimeCommands);
		// TODO: So, runtimeCommands isn't actually running on it's own thread- you're just calling
		// run(); you'd need to call start()
		runtimeCommands.run();
	...
	/**
	 * Iterate through the list of peers, and create a new list that only meets the specified
	 * requirements.
	 * 
	 * @param trackerPeers
	 */
	public void populatePeers(ArrayList<Peer> trackerPeers) {
		...
		// Iterate through the list of peers, and add them to new list if they match the specified
		// requirements.
		for (int currPeerIndex = 0; currPeerIndex < trackerPeers.size(); currPeerIndex++) {
			...
			// Rob's filter
			if (id.substring(0, 6).equals("RUBT11") && (currPeer.getIp().equals("128.6.5.130")
					|| currPeer.getIp().equals("128.6.5.131"))) {
	...
	/**
	 * Checks to see if we've successfully received all pieces of the file so we can send Event
	 * Complete to Tracker.
	 * 
	 * @return
	 */
	...
	public void exit() throws IOException, InterruptedException {
		...
		// Sleep long enough for our message handlers to finish up and our file access to close
		// properly
		Thread.sleep(1000L);
	...
	/**
	 * To be used in the event that the program exited before completing the download the prior time
	 * it was run
	 * 
	 * @param myPieces
	 */
	...
	/**
	 * Since TrackerStats does not take Manager as a param, in order to display correct amount
	 * downloaded, we must set it from Manager.
	 */
