...
public class Tracker implements Runnable {
	...
	/**
	 * The port number the user will listen on. Conventionally, we'll start by trying to connect to
	 * port 6881, then 6882, then 6883, etc, until port 6889 where we will give up trying to
	 * connect.
	 */
	private int port;
	...
	/**
	 * This is an optional key which maps to started , completed , or stopped (or empty , which is
	 * the same as not being present. If not present, this is one of the announcements done at
	 * regular intervals. An announcement using started is sent when a download first begins, and
	 * one using completed is sent when the download is complete. No completed is sent if the file
	 * was complete when started. Downloaders send an announcement using stopped when they cease
	 * downloading.
	 */
	private byte[] event;
	 ...
	 */
	public final static ByteBuffer KEY_INTERVAL = ByteBuffer
			.wrap(new byte[]{'i', 'n', 't', 'e', 'r', 'v', 'a', 'l'});

	/**
	 * Key used to retrieve the peers.
	 */
	public final static ByteBuffer KEY_PEERS = ByteBuffer
	...
	public final static byte[] EVENT_COMPLETED = new byte[]{'c', 'o', 'm', 'p', 'l', 'e', 't', 'e',
			'd'};
	 ...
	 * 
	 * @param torrentInfo
	 *            TorrentInfo object containing all information provided by torrent file.
	 * @param peerID
	 *            byte[] containing a generated user peerID.
	 */
	 ...
	 * 
	 * @throws IOException
	 *             The URL was improperly created.
	 */
	...
	/**
	 * Connects to the tracker, and returns the Bencoded dictionary containing the keys: interval
	 * and peers.
	 * 
	 * @throws IOException
	 */
	...
	private void decodeTrackerResponse() throws BencodingException, IOException {
		...
		ArrayList<Map<ByteBuffer, Object>> peerMapList = (ArrayList<Map<ByteBuffer, Object>>) this.trackerResponseMap
				.get(KEY_PEERS);

		// Loops through the list of peer dictionaries and uses the info to construct a list of Peer
		// objects.
	...
	// TODO: horribly inefficient hack- would be much better to replace the arrayList of peers we
	// use for a hashmap, this doesn't belong in tracker either but i dont know where else to put it
	/**
	 * Looks thru array list to see if the specified peer's ID has a match; returns true if we have
	 * the peer in the list and false otherwise.
	 */
	...
	/**
	 * Sets the event parameter by specified byte array. Can use the keys supplied at top of class,
	 * EVENT_STOPPED, EVENT_COMPLETED, and EVENT_STARTED
	 * 
	 * @param eventStatus
	 * @return True if successful, false if otherwise
	 */
	public boolean setEvent(byte[] eventStatus) {
		if (eventStatus == EVENT_STOPPED || eventStatus == EVENT_COMPLETED
	...
	public int getLeft() {
		// TODO: isDownloadComplete sends sets a EVENT_STOPPED if it is, which we only want to send
		// ONCE, the FIRST time we finish download
