...
/**
 * Message class represents a Peer-to-Peer message in the protocol (Unchoke, Interested, Piece,
 * etc.). Has static methods to encode and decode to/from Input/Output streams. Handles all p2p
 * communication.
 * 
 *
 * @author Dylan Murray
...
public class Message {
	...
	// KEEPALIVE messages don't actually have an id - this byte identification is for local
	// convenience only.
	...
	/**
	 * Sends a handshake out and expects a handshake back. If it does not receive a handshake back
	 * or if the peer ID/ infoHash is not as expected, it will return false.
	 * 
	 * @param in
	 * @param out
	 * @param handshake
	 ...
	 * @return
	 */
	// public static boolean doHandshake(DataInputStream in, DataOutputStream out, byte[] handshake,
	...
	/**
	 * Method used to decode incoming message from peer. Returns a Message object with all relevant
	 * information.
	 * 
	 * @param in
	 * @return
	...
	public static Message decode(DataInputStream in) {
		...
		try {
			...
			// We checked for Keep Alive, so the next byte will be the id. Find out what message
			// we're dealing with.
			switch (in.readByte()) {
				case CHOKE :
	...
	/**
	 * Generates the generic message in byte array format. (ie Choke, Unchoke, Interested, Not
	 * interested. Special messages will override).
	 * 
	 * @return
	 */
